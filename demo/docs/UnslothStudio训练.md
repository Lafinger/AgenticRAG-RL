...
| `data\novel_eval\sft_zh_unsloth\eval.json` | JSON 数组，每项包含 `messages` | Studio 验证集，与 `train_studio.jsonl` 不重叠 |
...
- 当前 `eval.json` 是 Studio 验证集；`test.jsonl` 是最终 QA 测试集。
...
eval_output = Path("data/novel_eval/sft_zh_unsloth/eval.json")
...
eval_payload = json.dumps(eval_records, ensure_ascii=False, indent=2)
with eval_output.open("w", encoding="utf-8", newline="") as handle:
    handle.write(eval_payload.replace("\n", "\r\n"))
    handle.write("\r\n")
...
如果 loss 继续降到 `<0.1`，但没有 eval loss，需要警惕模型只是记住训练轨迹。建议始终配置独立 `eval.json` 并观察 eval loss 是否同步下降。
...
```text
train.jsonl + eval.json
```

则会包含，因为 `eval.json` 是从全量 `train.jsonl` 中切出来的。因此 Studio 训练时不要再使用全量 `train.jsonl`。
