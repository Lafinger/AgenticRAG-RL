...
主 SFT、Agent loop 和后续 GRPO 共享同一套 Qwen3 tool schema。当前暴露给模型的 schema 包含 `keyword_search`、`dense_search`、`hybrid_search`；`graph_search` 仍可作为索引和检索实现能力保留，但不进入 canonical ReAct SFT 主 schema。Oracle trace 默认每跳使用 `keyword_search` 命中 gold chunk，评测和 reward 再用 `gold_chunks` / `retrieved_chunk_ids` 检查索引召回是否对齐。
