...
v4 SFT 数据会把 tool response 中的单条证据文本截断到固定上限，保证 `max_seq_length: 2048` 下不截断训练样本。截断只影响模型上下文长度，不改变 `chunk_id`、索引、gold chunk 或评测召回口径。v4 新增的 `loss_weights` 只改变训练目标权重，不改变 corpus、index 或 gold chunk 口径。
