...
`--max-concurrency` 只影响 KG 抽取时并发调用豆包大模型的数量。FAISS 和 BM25 构建不使用豆包。
...
- KG 需要调用豆包，大模型输出有概率失败、超时或返回非 JSON，排查变量更多。
...
如果不加 `--skip-kg`，`build_index.py` 会为每个 chunk 调用豆包大模型抽取三元组：
...
- `status=failed` 的 chunk 会重新请求豆包。
- 未出现于 cache 的 chunk 也会请求豆包。
