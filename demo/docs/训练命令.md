...
下面命令会保留全量 `train.jsonl` 不变，并从中等间隔切出 200 条作为独立验证集。Studio 训练时使用 `train_studio.jsonl`，Eval dataset 使用 `eval.json`；两者样本不重叠。
...
这份 `eval.json` 与 `train_studio.jsonl` 不重叠，适合在 Studio 中观察 eval loss。正式泛化评测仍应使用 `eval_hf_model.py`、`compare_predictions.py` 和 `run_llm_judge.py` 对 held-out QA 集做训练前后对比。
