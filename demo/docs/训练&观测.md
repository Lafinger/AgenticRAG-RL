...
v4 训练入口还会派生 `loss_weights`：assistant 首 token、`<think>`、`</think>`、`<tool_call>`、`<answer>`、`</answer>` 和 `<|im_end|>` 会被加权监督，`</tool_call>` 会降权，避免继续放大 closing-tag 先验。若 smoke 测评仍出现首轮直接输出 `</tool_call>`，不要进入完整训练，先重新跑首 token probe 并使用当前边界权重重新 smoke。
