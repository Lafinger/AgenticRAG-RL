...
本文只记录 demo 当前保留的主线训练流程：canonical ReAct SFT v4 -> Unsloth LoRA -> merged model -> Agent loop 测评。所有训练入口都通过命令行执行。
...
v4 训练入口还会派生 `loss_weights`：assistant 首 token、`<think>`、`<answer>` 和 `<|im_end|>` 会被加权监督，`</tool_call>` 不额外加权，避免继续放大 closing-tag 先验。
...
重点看 `first_token_probe` 中 `<think>`、`<answer>`、`<tool_call>`、`</tool_call>` 的概率和 `top1_closing_tool_rate`。主线期望无锚点 prompt 下 `</tool_call>` 不再成为高概率首 token。
...
完整训练前建议先跑 v4 smoke 训练，确认协议边界加权 loss 能让模型在训练样本 prompt 上稳定生成 `<think>`：
...
smoke checkpoint 先用 `docs\训练测评.md` 的 5 条 Agent loop 命令检查：`starts_with_closing_tool_rate <= 0.05`、`think_tag_rate >= 0.90`、`valid_tool_call_rate >= 0.80` 后，再跑完整训练。
...
uv run --no-sync python `
  ./scripts/export_unsloth_lora.py `
  --config ./training/unsloth_sft_v4.yaml `
  --adapter-path ./training/outputs/unsloth_sft_qwen3_4b_lora_react_v4/checkpoint-735 `
  --export-dir ./models/Qwen3-4B-Instruct-2507-Unsloth-SFT-react-v4-ckpt735-merged
```
...
4. 50 条 Agent loop 中 `think_tag_rate >= 0.95`、`valid_tool_call_rate >= 0.95`、`answer_tag_rate >= 0.80`、`starts_with_closing_tool_rate <= 0.05`。
