...
| `data/novel_eval/sft_zh_unsloth/eval.json` | Studio train/eval 独立切分 | Unsloth Studio eval loss 监控 |
...
- 如果使用 Unsloth Studio，应再从全量 `train.jsonl` 切出不重叠的 `train_studio.jsonl` 和 `eval.json`，避免验证集同时出现在 Studio 训练集中。
...
flowchart LR
    ...
    B --> D["manifest.json"]
    C --> E["Unsloth SFT"]
    C --> F["train_studio.jsonl + eval.json"]
    F --> G["Unsloth Studio"]
```
...
- `data/novel_eval/sft_zh_unsloth/eval.json`，仅用于 Unsloth Studio eval loss
...
| `eval.json` | JSON 数组，每项包含 `messages[]` | 与 `train_studio.jsonl` 不重叠的 Studio 验证集 | Unsloth Studio eval dataset |
...
- Unsloth Studio 建议使用独立切分后的 `data/novel_eval/sft_zh_unsloth/train_studio.jsonl` 和 `data/novel_eval/sft_zh_unsloth/eval.json`。
...
- Studio 验证文件：`data/novel_eval/sft_zh_unsloth/eval.json`
...
```powershell
uv run python .\scripts\convert_sft_to_unsloth.py --input-dir .\data\novel_eval\sft --output-dir .\data\novel_eval\sft_zh_unsloth
# 如果使用 Unsloth Studio，先按 docs\UnslothStudio训练.md 生成互不重叠的 train_studio.jsonl 和 eval.json
