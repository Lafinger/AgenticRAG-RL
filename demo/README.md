...
| `docs/训练&观测.md` | Unsloth SFT/GRPO 训练命令、SwanLab 和本地 dashboard 观测入口 |
...
| `data/novel_eval/sft_zh_unsloth_react_v3/train.jsonl` | Step 8 Unsloth v3 数据导出 | SFT 全量 messages 数据 |
| `data/novel_eval/sft_zh_unsloth_react_v3/train_cli.jsonl` | 命令行 train/eval 独立切分 | CLI SFT 训练输入 |
| `data/novel_eval/sft_zh_unsloth_react_v3/eval.jsonl` | 命令行 train/eval 独立切分 | CLI SFT eval loss 监控 |
| `data/novel_eval/grpo_agentic_*.parquet` | Step 9 GRPO 数据构造 | GRPO/RL 训练输入与 reward ground truth |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v3` | Step 12 SFT LoRA 训练 | LoRA adapter，供合并或 adapter 方式测评 |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v3_trace/step_sample_trace.jsonl` | 可追溯 SFT LoRA 训练 | optimizer step、loss、grad norm 到训练样本的映射 |
...
| 训练 | `scripts/train_sft_unsloth.py`、`scripts/train_sft_unsloth_trace.py`、`scripts/export_unsloth_lora.py`、`scripts/train_grpo_unsloth.py` | SFT LoRA、可追溯 SFT LoRA、LoRA 合并、GRPO/RL 训练入口 |
...
- `data/novel_eval/sft_react_v3/react.jsonl`
- `data/novel_eval/sft_react_v3/sharegpt.jsonl`
- `data/novel_eval/sft_react_v3/messages.jsonl`
...
- `convert_sft_to_unsloth.py` 读取 v3 `sharegpt.jsonl`，保留 `tool` role、record 级 `tools` 字段和 assistant message 级 `loss` 元数据，转换成项目训练入口使用的 `train.jsonl`。
...
- `training/unsloth_sft.yaml` 直接引用 `data/novel_eval/sft_zh_unsloth_react_v3/train_cli.jsonl` 和 `data/novel_eval/sft_zh_unsloth_react_v3/eval.jsonl`，从头训练时会按 `eval_steps` 记录 eval loss。
...
- 输入目录：`data/novel_eval/sft_react_v3/`
- 脚本：`scripts/convert_sft_to_unsloth.py`
- 输出目录：`data/novel_eval/sft_zh_unsloth_react_v3/`
...
- `data/novel_eval/sft_zh_unsloth_react_v3/train.jsonl`
- `data/novel_eval/sft_zh_unsloth_react_v3/train_cli.jsonl`，用于命令行 train/eval 切分后的训练
- `data/novel_eval/sft_zh_unsloth_react_v3/eval.jsonl`，用于命令行 eval loss 验证
- `data/novel_eval/sft_zh_unsloth_react_v3/manifest.json`
...
当前 v3 默认切分为 `16361` 条 CLI 训练样本和 `200` 条 CLI 验证样本，二者不重叠；训练集包含 `full_trace`、`first_action_only`、`next_action_only` 和 `final_answer_only`。其中 `first_action_only` 约 2 倍加权，用于修复首轮生成 `</tool_call>` 的协议崩溃。训练、eval loss、trace 追溯和 LoRA 合并命令统一见 `docs/训练&观测.md`。
...
- `tools` 字段写入同一份 Qwen3 tool schema，后续 GRPO rollout 可和 SFT/Agent loop 共用工具模板。
...
| `tools` | Qwen3 tool schema | GRPO rollout 工具模板 |
...
- 项目脚本默认使用 Step 8 切分出的命令行训练集和验证集，即 `data/novel_eval/sft_zh_unsloth_react_v3/train_cli.jsonl` 和 `data/novel_eval/sft_zh_unsloth_react_v3/eval.jsonl`。
...
- LoRA 只训练 adapter 权重，默认输出到 `training/outputs/unsloth_sft_qwen3_4b_lora_react_v3`。
- 合并阶段把 LoRA adapter 写回基座模型，生成后续 GRPO 默认使用的 merged model；v2 旧 checkpoint 630/720/765/1089 已确认为首轮 `</tool_call>` 协议失败基线，不再作为有效主结果。
...
- LoRA 输出：`training/outputs/unsloth_sft_qwen3_4b_lora_react_v3`
- 可追溯训练输出：`training/outputs/unsloth_sft_qwen3_4b_lora_react_v3_trace`
...
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v3` | LoRA adapter | v3 主 SFT adapter | export |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v3/metrics.jsonl` | Trainer JSONL metrics | dashboard 标准曲线 | `docs/训练&观测.md` |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v3/swanlab_history_replay_state.json` | SwanLab 历史回放审计 | 记录回放到哪个 run 和 step，避免重复上传 | `docs/训练&观测.md` |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v3_trace/step_sample_trace.jsonl` | JSONL trace | 每个 optimizer step 对应的样本、loss、lr、grad norm、eval loss | 突刺追溯 |
...
- 启动 retrieval server、运行 GRPO 和查看 SwanLab/dashboard 的命令见 `docs/训练&观测.md`。
...
- GRPO SwanLab 云端实验：`agentic-rag-rl`
- GRPO dashboard 指标：`training/outputs/unsloth_grpo_qwen3_4b/metrics.jsonl`
- 可进入后续 Agentic 评测
