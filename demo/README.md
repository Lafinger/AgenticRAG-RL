...
- 外部 Linux / WSL / A100 环境的 Unsloth SFT 与 verl tool-agent GRPO 训练入口
...
| `docs/SFT_LORA/SFT_LORA数据流.md` | 说明 corpus、QA、Oracle trace 和 SFT messages 的主数据流 |
| `docs/SFT_LORA/SFT_LORA训练&观测.md` | Unsloth SFT、SwanLab 和本地 dashboard 观测入口 |
| `docs/SFT_LORA/SFT_LORA训练追溯.md` | SFT/QLoRA 按 optimizer step 精确追溯样本、定位 Gradient Norm 突刺 |
| `docs/SFT_LORA/SFT_LORA训练测评.md` | Base、SFT LoRA 和 merged model 的 Agent loop 测评方法 |
| `docs/SFT_LORA/SFT_LORA训练样本长度计算.md` | 统计 SFT 样本 token 长度，辅助设置 `max_seq_length` |
| `docs/SFT_LORA/SFT_LORA资源推算.md` | 估算 SFT LoRA / QLoRA 的显存和训练步数 |
| `docs/SFT_LORA/SFT_LORA训练改动.md` | 记录 SFT V1-V4 协议演进、失败原因和当前有效基线 |
| `docs/RL/RL数据流.md` | 说明 GRPO parquet schema、工具配置和 verl tool-agent 消费方式 |
| `docs/RL/RL训练&观测.md` | retrieval server、verl GRPO 启动命令、核心参数和日志观测 |
| `docs/RL/RL训练测评.md` | GRPO 后 smoke / 50 条主测评和 SFT v4 退化门槛 |
| `docs/RL/RL训练样本长度计算.md` | 估算 rollout、tool response、batch 和上下文长度预算 |
| `docs/RL/RL训练追溯.md` | 当前 RL 的日志、raw turns、reward 和检索追溯口径 |
| `docs/RL/RL_资源推算.md` | 估算 GRPO 多卡、rollout token 和 retrieval server 资源 |
| `docs/RL/RL训练改动.md` | 记录 GRPO 从旧 TRL/Unsloth 壳切到 example 风格 verl tool-agent 的改动 |
| `docs/FQA.md` | 解释训练样本、label mask、协议边界和常见问题 |
...
| `data/novel_eval/sft_zh_unsloth_react_v4/train.jsonl` | Step 8 Unsloth v4 数据导出 | SFT 全量 messages 数据 |
| `data/novel_eval/sft_zh_unsloth_react_v4/train_cli.jsonl` | 命令行 train/eval 独立切分 | CLI SFT 训练输入 |
| `data/novel_eval/sft_zh_unsloth_react_v4/eval.jsonl` | 命令行 train/eval 独立切分 | CLI SFT eval loss 监控 |
| `data/novel_eval/grpo_agentic_*.parquet` | Step 9 GRPO 数据构造 | GRPO/RL 训练输入与 reward ground truth |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v4` | Step 12 SFT LoRA 训练 | LoRA adapter，供合并或 adapter 方式测评 |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v4_trace/step_sample_trace.jsonl` | 可追溯 SFT LoRA 训练 | optimizer step、loss、grad norm 到训练样本的映射 |
| `models/Qwen3-4B-Instruct-2507-Unsloth-SFT-react-v4-merged` | Step 12 LoRA 合并 | GRPO 初始模型、SFT 后生成评测模型 |
...
| SFT/GRPO 数据 | `scripts/build_oracle_traces.py`、`scripts/trace_to_sft.py`、`scripts/convert_sft_to_unsloth.py`、`scripts/split_sft_train_eval.py`、`scripts/prepare_agentic_grpo_data.py` | 构造 Oracle traces、SFT JSONL、CLI train/eval 切分和 GRPO parquet |
| 训练 | `scripts/train_sft_unsloth.py`、`scripts/train_sft_unsloth_trace.py`、`scripts/export_unsloth_lora.py`、`training/start_grpo_tool_agent.sh` | SFT LoRA、可追溯 SFT LoRA、LoRA 合并、verl multi-turn tool-agent GRPO 训练入口 |
| 训练观测 | `scripts/watch_unsloth_training.py`、`training/monitoring.py` | SwanLab 云端标量指标和本地 JSONL dashboard |
| 评测 | `scripts/eval_agentic.py`、`scripts/run_cloud_eval.py`、`scripts/eval_hf_agentic.py`、`scripts/compare_predictions.py`、`scripts/run_llm_judge.py` | 本机 smoke、HF Agent loop、结果对比和 LLM-as-Judge |
| 训练追溯 | `scripts/inspect_sft_trace.py` | 按 step 或 top grad norm 反查触发突刺的训练样本 |
...
- 未加 `--skip-kg` 时，脚本会并发调用所选在线模型供应商抽取三元组，构建 `knowledge_graph.json` 和 `entity_embeddings.pkl`；默认最大并发为 `5`，可通过 `--max-concurrency` 调整。
...
- KG 抽取会把每个 chunk 的状态逐条追加到 `triples_cache.jsonl`：成功写 `status=ok`，失败写 `status=failed`。重新执行同一命令时只跳过 `ok` 的 chunk，失败和未完成 chunk 会重新请求在线模型；最终缓存会按 chunk 顺序重写。
...
- 脚本默认启用断点续写：`seeds.checkpoint.jsonl` 记录每个 chunk 的 `status=ok/failed`；重新执行同一命令时只跳过 `ok` 的 chunk，失败和未完成 chunk 会重新请求在线模型。
- 如果模型返回非法 JSON 或单个 chunk 多次失败，脚本默认把失败 chunk 写入 `seeds.failed.jsonl` 并继续后续 chunk，避免整个任务中断。
...
- 每条 seed 的 `question/answer/qa_type/entities` 来自在线模型，`doc_chunk_id/tool` 由脚本补齐
...
| 合并模型 | 强模型 merge | 默认 `gpt-5.5` 做自然问题合并 |
...
- 多跳合并默认使用所选在线模型，生成 `final_question/final_answer/qa_type/answer_aliases`；当前 v1 强制 `final_answer` 等于最后一跳 `answer`。
...
- 可设置 `--candidate-multiplier -1` 使用旧补样语义：持续尝试候选，直到通过样本达到 `--target-count` 或候选耗尽。
...
- `gen_enhanced_aliases.py` 当前是规则处理，不调用在线模型，因此没有独立 Prompt。
...
- 对每个 hop，脚本会生成一条 assistant ReAct 输出：短 `<think>` 加换行版 `<tool_call>` JSON payload；对应 chunk 保留为 `tool` role 原始检索文本。
...
**目标**：把 Oracle traces 转换成 tool-aware canonical ReAct SFT 记录，供 Unsloth 项目脚本使用。
...
- `data/novel_eval/sft_react_v4/react.jsonl`
- `data/novel_eval/sft_react_v4/sharegpt.jsonl`
- `data/novel_eval/sft_react_v4/messages.jsonl`
...
| 产物 | 结构 / 字段 | 含义 | 后续使用位置 |
| --- | --- | --- | --- |
| `react.jsonl` | `messages/tools/metadata` | canonical ReAct 主数据 | 调试、协议检查 |
| `sharegpt.jsonl` | `messages/tools/metadata` | Unsloth 项目脚本入口 | Step 8 |
| `messages.jsonl` | `messages/tools/metadata` | 兼容不同数据入口命名 | SFT 训练 |
| `messages[].role` | system/user/assistant/tool | 标识对话角色 | tokenizer / template |
| `messages[].content` | 消息正文 | 包含工具调用和答案标签 | SFT loss |
| `tools` | Qwen3 tool schema | 自动注入 `# Tools` 与 `<tools>` 段 | SFT/Agent loop/GRPO |
...
**目标**：把 tool-aware canonical ReAct SFT 数据打包成 Unsloth 项目脚本可识别的数据集目录。

**详细说明**：

- `convert_sft_to_unsloth.py` 读取 v4 `sharegpt.jsonl`，保留 `tool` role、record 级 `tools` 字段和 assistant message 级 `loss` 元数据，转换成项目训练入口使用的 `train.jsonl`。
...
- `training/unsloth_sft_v4.yaml` 和当前默认 `training/unsloth_sft.yaml` 直接引用 `data/novel_eval/sft_zh_unsloth_react_v4/train_cli.jsonl` 和 `data/novel_eval/sft_zh_unsloth_react_v4/eval.jsonl`，从头训练时会按 `eval_steps` 记录 eval loss。
- `eval.jsonl` 是命令行训练和 trace eval 的通用验证集，不使用最终 held-out `test.jsonl`。
...
- 输入目录：`data/novel_eval/sft_react_v4/`
- 脚本：`scripts/convert_sft_to_unsloth.py`
- 输出目录：`data/novel_eval/sft_zh_unsloth_react_v4/`
...
- `data/novel_eval/sft_zh_unsloth_react_v4/train.jsonl`
- `data/novel_eval/sft_zh_unsloth_react_v4/train_cli.jsonl`，用于命令行 train/eval 切分后的训练
- `data/novel_eval/sft_zh_unsloth_react_v4/eval.jsonl`，用于命令行 eval loss 验证
- `data/novel_eval/sft_zh_unsloth_react_v4/manifest.json`
- SFT 配置：`training/unsloth_sft_v4.yaml`
...
当前 v4 全量导出为 `19561` 条 records，默认切分为 `19361` 条 CLI 训练样本和 `200` 条 CLI 验证样本，二者不重叠；训练集包含 `full_trace`、`first_action_only`、`next_action_only` 和 `final_answer_only`。v4 不再只靠增加样本数修复协议问题，而是在训练入口为 assistant 首 token、`<think>`、`</think>`、`<tool_call>`、`<answer>`、`</answer>` 和 `<|im_end|>` 派生 `loss_weights` 做协议边界加权监督；`</tool_call>` 会降权，避免继续放大 closing-tag 先验。训练、eval loss、trace 追溯和 LoRA 合并命令统一见 `docs/SFT_LORA/SFT_LORA训练&观测.md`。

## Step 9: 构造 verl tool-agent GRPO parquet 数据

**目标**：把多跳 QA 转换成 example 风格的 verl multi-turn tool-agent GRPO parquet，保留 raw chat、reward ground truth 和 tool-agent 运行所需 `tools_kwargs`。

**详细说明**：

- `prepare_agentic_grpo_data.py` 读取多跳 QA，并调用 `build_grpo_rows` 构造 GRPO 训练行。
- 每条样本只把 system prompt 和用户问题写入 `prompt`，不泄漏 oracle evidence 或 gold chunks。
- parquet schema 对齐 example 主线：`data_source/prompt/ability/reward_model/extra_info/metadata`。
- 工具 schema 不再写入 parquet；verl 训练时从 `training/config/novel_tool_config.yaml` 加载 `KeywordSearchTool/DenseSearchTool/HybridSearchTool`。
- `extra_info.need_tools_kwargs=True`，`extra_info.tools_kwargs` 为每个工具注入 `question/ground_truth/data_source`。
...
- GRPO train parquet
- GRPO val parquet
- 每行包含 `data_source/prompt/ability/reward_model/extra_info/metadata`
- `ground_truth` 包含 `target/answer/question/answer_aliases/gold_chunks/hop_count`
- `extra_info.tools_kwargs` 对齐 verl tool-agent 的工具实例创建参数

**数据结构与字段用途**：

| 字段 | 含义 | 后续使用位置 |
| --- | --- | --- |
| `data_source` | 固定 `novel_agentic_rag` | reward/debug |
| `prompt` | 原始 chat prompt，包含 system 和 user question | verl GRPO 输入 |
| `ability` | 固定 `multi_hop_qa` | 任务类型标识 |
...
| `extra_info.need_tools_kwargs` | 固定 `true` | verl tool-agent 工具参数注入 |
| `extra_info.tools_kwargs` | 每个工具的 create kwargs | 工具实例上下文 |
| `metadata.tool_names` | `keyword_search/dense_search/hybrid_search` | 数据审计 |
...
| Tool config | 工具定义由 `training/config/novel_tool_config.yaml` 统一维护，不写入 parquet |
...
**目标**：为 verl tool-agent GRPO 的多轮工具调用提供 HTTP 检索工具服务。

**详细说明**：

- `retrieval_server.py` 启动 FastAPI 服务；正式 GRPO 前必须带 `--index-dir` 使用已构建索引，避免退化为 corpus-only fallback。
- 服务启动后会构造 indexed retriever，支持 keyword、dense 和 hybrid 三类检索请求。
- `/search` 接口接收 `query/top_k/tool`，返回 chunk ID、标题、文本、分数和 metadata。
- `training/tools/novel_search_tool.py` 中的 verl `BaseTool` 会调用该 `/search` 接口，并把结果格式化为 `[chunk_id] text` 的 tool response。
- 本机 Windows 可运行服务和 smoke；完整 GRPO 训练推荐 Linux/WSL/远程 GPU 的 verl 环境。
...
- verl `KeywordSearchTool/DenseSearchTool/HybridSearchTool` 通过该 HTTP 服务接入检索工具
...
| 返回证据 | server 返回 `chunk_id/title/text/score/metadata`；BaseTool 暴露给模型的文本为 `[chunk_id] text` |
...
- 项目脚本默认使用 Step 8 切分出的命令行训练集和验证集，即 `data/novel_eval/sft_zh_unsloth_react_v4/train_cli.jsonl` 和 `data/novel_eval/sft_zh_unsloth_react_v4/eval.jsonl`。
...
- 标准训练、可追溯训练、SwanLab、本地 dashboard 和 LoRA 合并命令统一见 `docs/SFT_LORA/SFT_LORA训练&观测.md`。
- 训练后测评见 `docs/SFT_LORA/SFT_LORA训练测评.md`，按 step 追溯异常样本见 `docs/SFT_LORA/SFT_LORA训练追溯.md`。
...
- LoRA 输出：`training/outputs/unsloth_sft_qwen3_4b_lora_react_v4`
- 可追溯训练输出：`training/outputs/unsloth_sft_qwen3_4b_lora_react_v4_trace`
...
- 合并模型：`models/Qwen3-4B-Instruct-2507-Unsloth-SFT-react-v4-merged`
...
| --- | --- | --- | --- |
| `training/unsloth_sft_v4.yaml` | model/dataset/template/output_dir | v4 SFT 训练参数 | Unsloth |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v4` | LoRA adapter | v4 主 SFT adapter | export |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v4/metrics.jsonl` | Trainer JSONL metrics | dashboard 标准曲线 | `docs/SFT_LORA/SFT_LORA训练&观测.md` |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v4/swanlab_history_replay_state.json` | SwanLab 历史回放审计 | 记录回放到哪个 run 和 step，避免重复上传 | `docs/SFT_LORA/SFT_LORA训练&观测.md` |
...
| SwanLab `agentic-rag-rl` | 云端实验 | loss、eval loss、grad norm、learning rate 曲线 | `docs/SFT_LORA/SFT_LORA训练&观测.md` |
...
## Step 13: verl multi-turn tool-agent GRPO 训练

**目标**：严格复刻 example 的 multi-turn tool-agent GRPO：使用 verl + vLLM + Ray 进行多轮 rollout，模型真实生成 `<think> + <tool_call>`，tool-agent 调用小说检索服务返回 `<tool_response>`，reward 根据最终 `<answer>`、检索命中和答案质量打分。

**详细说明**：

- GRPO 使用 SFT v4 merged model 作为初始模型：`models/Qwen3-4B-Instruct-2507-Unsloth-SFT-react-v4-merged`。
- 正式 GRPO 主入口是 `training/start_grpo_tool_agent.sh`，不是旧的 `scripts/train_grpo_unsloth.py`。
- 训练脚本复刻 example 的 verl hydra 主参数：`algorithm.adv_estimator=grpo`、`data.return_raw_chat=True`、`actor_rollout_ref.rollout.agent.default_agent_loop=tool_agent`、`actor_rollout_ref.rollout.multi_turn.enable=True`、`actor_rollout_ref.rollout.multi_turn.format=hermes`。
- 工具配置由 `training/config/novel_tool_config.yaml` 提供，加载 `training.tools.novel_search_tool.KeywordSearchTool/DenseSearchTool/HybridSearchTool`。
- 每个工具只接收 `{"query": "..."}`，内部统一调用 `http://127.0.0.1:8790/search`。
- reward 函数只从 `<answer>...</answer>` 抽取预测答案，不把无标签文本当答案；JSON `<tool_call>`、`<tool_response>` 和 `[chunk_id]` 用于检索质量与搜索充分性打分。
- Windows 本机负责数据、检索服务、单测和小样本 smoke；完整 GRPO 推荐在 Linux/WSL/远程多卡 GPU 环境运行 verl。

```mermaid
flowchart TD
    A["SFT v4 merged model"] --> B["verl main_ppo / tool_agent"]
    C["GRPO train / val parquet"] --> B
    D["novel_tool_config.yaml"] --> B
    E["retrieval_server /search"] --> B
    F["reward_agentic_rag.py"] --> B
    B --> G["GRPO checkpoint"]
```
...
- Linux/WSL/远程 GPU 环境
- verl、vLLM、Ray、torch、transformers
...
- 启动脚本：`training/start_grpo_tool_agent.sh`
- Reward：`training/reward_agentic_rag.py`

**能拿到的结果**：

- GRPO checkpoint：`training/outputs/grpo_tool_agent_react_v4`
- GRPO 日志：`logs/grpo_tool_agent_react_v4.log`
- 可进入后续 Agentic 评测
...
| 组件 | 结构 / 字段 | 含义 | 后续使用位置 |
| --- | --- | --- | --- |
| `grpo_agentic_train.parquet` | `data_source/prompt/ability/reward_model/extra_info/metadata` | 训练 rollout 数据 | verl GRPO |
| `grpo_agentic_val.parquet` | 同 train parquet | 验证 rollout 数据 | checkpoint 选择 |
| `novel_tool_config.yaml` | verl `BaseTool` class + schema | tool-agent 工具定义 | multi-turn rollout |
| `reward_agentic_rag.py` | `compute_score` | 自定义 reward 入口 | GRPO reward function |
| `AGENTIC_RAG_REWARD_VERSION` | 默认 `v9a` | 切换 reward 公式 | GRPO/RL |
| `start_grpo_tool_agent.sh` | verl `main_ppo` hydra overrides | 正式 GRPO 入口 | Linux/WSL/远程 GPU |
...
| 工具 JSON | 工具调用必须符合 canonical Qwen3 ReAct JSON 格式，字段为 `name` 和 `arguments` |
...
| `think_tag_rate` | 工具轮是否稳定包含短 `<think>` |
| `malformed_think_rate` | `<think>` 是否残缺、重复或位置错误 |
| `hop_recall` | Agent loop 检索到的 chunk 是否覆盖 gold chunks |
| `max_turns_exceeded_rate` | Agent loop 是否因为不会停止回答而耗尽轮次 |
...
SFT LoRA 是 GRPO 前的冷启动阶段，建议在数据闭环 smoke 通过后单独执行。具体顺序固定为：

```text
确认 Unsloth 环境
必要时刷新 SFT messages 数据
统计样本长度
训练 LoRA 或可追溯 LoRA
合并 merged model
运行 Agent loop smoke 和 50 条 held-out 测评
```

训练和观测命令见 `docs\SFT_LORA\SFT_LORA训练&观测.md`。训练后主结论以 HF Agent loop 评测为准，checkpoint 选择和测评命令见 `docs\SFT_LORA\SFT_LORA训练测评.md`。
...
- 单卡 16GB：适合数据流程、小模型或 QLoRA smoke；当前默认 `Qwen3-4B + LoRA rank 64 + max_seq_length 2048`，仍需按显存情况观察 batch 和 LoRA rank。
- 单卡 24GB：更适合 4B LoRA SFT；GRPO 仍建议缩小 batch、response length 或只做 smoke。
- 外部 GPU 环境：完整 Unsloth SFT、verl tool-agent GRPO、长样本 rollout 和 Judge。
- 数据构造成本主要来自按 chunk 调用在线 LLM 的 KG 抽取和 seed QA 生成；正式全量构造前建议先小样本抽查质量，并按当前 provider 的实际价格预留预算。
