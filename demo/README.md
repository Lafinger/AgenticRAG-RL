...
| `docs/训练观测.md` | Unsloth 自定义 SFT/GRPO 训练的实时可视化工具方案 |
...
| 产物 | 结构 / 字段 | 含义 | 后续使用位置 |
| --- | --- | --- | --- |
| `train.jsonl` | 每行一个 `messages[]` 样本 | 全量 SFT 样本 | 项目脚本默认 train |
| `train_studio.jsonl` | 每行一个 `messages[]` 样本 | 已移除验证样本的 Studio 训练集 | Unsloth Studio train dataset |
| `eval.jsonl` | 每行一个 `messages[]` 样本 | 与 `train_studio.jsonl` 不重叠的 Studio 验证集 | Unsloth Studio eval dataset |
...
- 如果需要精确追溯 Gradient Norm 突刺对应的训练样本，使用 `scripts/train_sft_unsloth_trace.py` 重新跑可追溯训练；Studio 图表只能定位 step，不能可靠反推出样本。
- 可追溯训练的使用方法和 trace 字段说明见 `docs/训练追溯.md`。
...
- 可追溯训练脚本：`scripts/train_sft_unsloth_trace.py`
- 追溯查询脚本：`scripts/inspect_sft_trace.py`
...
如果本轮训练目标是追溯 Gradient Norm 突刺，改用可追溯训练入口：
...
如果要把可追溯训练得到的 adapter 作为冷启动模型导出，下一步 `--adapter-path` 改为 `.\training\outputs\unsloth_sft_qwen3_4b_lora_trace`。
...
| `training/outputs/unsloth_sft_qwen3_4b_lora/metrics.jsonl` | Trainer JSONL metrics | dashboard 标准曲线 | 训练观测 |
...
| SwanLab `agentic-rag-rl` | 云端实验 | loss、eval loss、grad norm、learning rate 曲线 | 训练观测 |
...
GRPO 同样默认向 SwanLab 云端项目 `agentic-rag-rl` 上报标量指标，并写出 `metrics.jsonl`。查看本地 dashboard：
...
```powershell
uv run python .\scripts\convert_sft_to_unsloth.py --input-dir .\data\novel_eval\sft --output-dir .\data\novel_eval\sft_zh_unsloth
uv run python .\scripts\split_unsloth_studio_data.py --input .\data\novel_eval\sft_zh_unsloth\train.jsonl --train-output .\data\novel_eval\sft_zh_unsloth\train_studio.jsonl --eval-output .\data\novel_eval\sft_zh_unsloth\eval.jsonl --manifest .\data\novel_eval\sft_zh_unsloth\manifest.json --eval-count 200
