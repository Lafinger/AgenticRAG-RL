...
| 训练观测 | `scripts/watch_unsloth_training.py`、`training/monitoring.py` | TensorBoard 指标输出和本地 JSONL dashboard |
...
训练默认写 TensorBoard events 和本地 JSONL 指标。另开 PowerShell 查看曲线：
...
| `training/runs/` | TensorBoard events | loss、eval loss、grad norm、learning rate 曲线 | TensorBoard |
...
GRPO 同样默认写 TensorBoard events 和 `metrics.jsonl`。查看本地 dashboard：
...
- GRPO TensorBoard 日志：`training/runs/unsloth_grpo_qwen3_4b`
