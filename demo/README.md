...
| `docs/UnslothStudio训练.md` | Unsloth Studio 的 QLoRA 训练配置、独立 train/eval 切分、loss 图表解释和收敛判断 |
...
| `data/novel_eval/sft_zh_unsloth/train.jsonl` | Step 8 Unsloth 数据导出 | SFT 全量 messages 数据 |
| `data/novel_eval/sft_zh_unsloth/train_studio.jsonl` | Studio train/eval 独立切分 | Unsloth Studio SFT 训练输入 |
| `data/novel_eval/sft_zh_unsloth/eval.jsonl` | Studio train/eval 独立切分 | Unsloth Studio eval loss 监控 |
...
| `training/outputs/unsloth_sft_qwen3_4b_lora_trace/step_sample_trace.jsonl` | 可追溯 SFT LoRA 训练 | optimizer step、loss、grad norm 到训练样本的映射 |
...
| SFT/GRPO 数据 | `scripts/build_oracle_traces.py`、`scripts/trace_to_sft.py`、`scripts/convert_sft_to_unsloth.py`、`scripts/split_unsloth_studio_data.py`、`scripts/prepare_agentic_grpo_data.py` | 构造 Oracle traces、SFT JSONL、Studio train/eval 切分和 GRPO parquet |
...
| 评测 | `scripts/eval_agentic.py`、`scripts/run_cloud_eval.py`、`scripts/eval_hf_model.py`、`scripts/eval_hf_agentic.py`、`scripts/compare_predictions.py`、`scripts/run_llm_judge.py` | 本机 smoke、模型生成对比、HF Agent loop 和 LLM-as-Judge |
...
- 对每个 hop，脚本会生成一条 assistant 的 ReAct 输出：`<think>...</think>` 加换行版 `<tool_call>`；对应 chunk 保留为 `tool` role 原始检索文本。
...
- 工具调用采用 Qwen3 tool template 对齐格式：`<tool_call>\n{"name": "...", "arguments": {...}}\n</tool_call>`
...
- 如果使用 Unsloth Studio，应再从全量 `train.jsonl` 切出不重叠的 `train_studio.jsonl` 和 `eval.jsonl`，避免验证集同时出现在 Studio 训练集中。
...
flowchart LR
    ...
    B --> D["manifest.json"]
    C --> E["Unsloth SFT"]
    C --> F["train_studio.jsonl + eval.jsonl"]
    F --> G["Unsloth Studio"]
```
...
如果使用 Unsloth Studio，用 Python 脚本切分互不重叠的训练集和验证集：
...
- `data/novel_eval/sft_zh_unsloth/train_studio.jsonl`，仅用于 Unsloth Studio 训练
- `data/novel_eval/sft_zh_unsloth/eval.jsonl`，仅用于 Unsloth Studio eval loss
...
| `train_studio.jsonl` | 每行一个 `messages/tools` 样本 | 已移除验证样本的 Studio 训练集 | Unsloth Studio train dataset |
| `eval.jsonl` | 每行一个 `messages/tools` 样本 | 与 `train_studio.jsonl` 不重叠的 Studio 验证集 | Unsloth Studio eval dataset |
| `manifest.json` | 数据格式、来源和 Studio split 摘要 | 记录转换来源、样本数和训练器 | 数据审计 |
| `training/unsloth_sft.yaml` | `dataset/template/model/output_dir` | SFT 训练配置 | Step 12 |

Unsloth Studio 的完整配置、train/eval 切分命令、loss 图表解释和收敛判断见 `docs/UnslothStudio训练.md`。当前默认切分为 `2800` 条 Studio 训练样本和 `200` 条 Studio 验证样本，二者不重叠。
...
- Unsloth Studio 建议使用独立切分后的 `data/novel_eval/sft_zh_unsloth/train_studio.jsonl` 和 `data/novel_eval/sft_zh_unsloth/eval.jsonl`。
- Studio 不能直接导入 `training/unsloth_sft.yaml`；需要按 `docs/UnslothStudio训练.md` 在 UI 中手动映射模型、数据、LoRA 和训练参数。
...
`eval_agentic.py` 和 `run_cloud_eval.py` 主要验证数据、检索和规则型 pipeline 是否闭环；它们不加载 SFT 后的 Qwen 模型。主 SFT 基线重训后，应以 `eval_hf_agentic.py --action-history-mode normalized` 作为 Agentic 结论入口：

- 主线 Agent loop 评测：`eval_hf_agentic.py --action-history-mode normalized`，衡量真实工具调用、检索闭环、`hop_recall` 和是否能用 `<answer>` 停止。
- 直接答案评测：`eval_hf_model.py --prompt-mode direct_answer`，仅作为无检索直答诊断，不用来否定 Agentic SFT。
- direct-answer / agentic-stop 数据和模型保留为补充路线，不能替代 canonical ReAct SFT 主基线。
...
- `results/sft_compare/sft_predictions.jsonl`，仅作为旧 direct-answer 诊断输出
...
训练 LoRA 或可追溯 LoRA
合并 merged model
先运行 Agent loop normalized 评测，再按需运行 direct-answer 诊断
```

训练和观测命令见 `docs\训练&观测.md`。训练后主结论以 HF Agent loop normalized 评测为准；direct-answer 和 agentic-stop 的命令见 `docs\训练测评.md`，只作为补充诊断路线。
