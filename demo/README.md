...
| `docs/数据流.md` | 说明 corpus、QA、SFT、GRPO parquet 的主数据流和字段 |
| `docs/环境安装.md` | Windows 11 + uv + CUDA Torch + Unsloth Core 安装与验证 |
| `docs/索引构建.md` | 解释 FAISS、BM25、KG、chunk store、RRF 和 rerank 的设计取舍 |
| `docs/训练&观测.md` | Unsloth SFT、verl tool-agent GRPO 训练命令、SwanLab 和本地 dashboard 观测入口 |
| `docs/训练追溯.md` | SFT/QLoRA 训练中按 optimizer step 精确追溯样本、定位 Gradient Norm 突刺 |
...
| `docs/SFT训练改动.md` | 记录 SFT V1-V4 协议演进、失败原因和当前有效基线 |
| `docs/GRPO训练改动.md` | 记录 GRPO 从旧 TRL/Unsloth 壳切到 example 风格 verl tool-agent 的改动 |
...
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v4/metrics.jsonl` | Trainer JSONL metrics | dashboard 标准曲线 | `docs/训练&观测.md` |
| `training/outputs/unsloth_sft_qwen3_4b_lora_react_v4/swanlab_history_replay_state.json` | SwanLab 历史回放审计 | 记录回放到哪个 run 和 step，避免重复上传 | `docs/训练&观测.md` |
...
训练和观测命令见 `docs\训练&观测.md`。训练后主结论以 HF Agent loop 评测为准，checkpoint 选择和测评命令见 `docs\训练测评.md`。
