...
- 当前训练入口默认启用 assistant-only loss mask，只对 assistant 输出计算 loss，system/user/tool_response 和 padding 都不参与 loss。
...
`eval_agentic.py` 和 `run_cloud_eval.py` 主要验证数据、检索和规则型 pipeline 是否闭环；它们不加载 SFT 后的 Qwen 模型。SFT LoRA 训练后应使用 `docs\训练测评.md` 中的两条评测线：

- 直接答案评测：`eval_hf_model.py --prompt-mode direct_answer`，衡量最终答案输出能力。
- Agent loop 评测：`eval_hf_agentic.py`，衡量真实工具调用、检索闭环和 `hop_recall`。
...
训练 LoRA 或可追溯 LoRA
合并 merged model
运行直接答案评测和 Agent loop 评测
```

训练和观测命令见 `docs\训练&观测.md`。训练后对比 base/SFT、direct-answer、HF Agent loop 和 LLM Judge 的命令见 `docs\训练测评.md`。
